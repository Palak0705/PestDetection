...
# Configure logging
log_file_path = r"Supervised_ResNet50_training_log.log"
...
def train_supervised(model, dataloader, optimizer, criterion, num_epochs, device):
    ...
    torch.save(model.state_dict(), "supervised_resnet50_model.pth")
    logging.info("Model saved to supervised_resnet50_model.pth")

from sklearn.metrics import classification_report

def evaluate(model, dataloader, device, class_names, export_path="evaluation_metrics.csv", classwise_export_path="classwise_performance.csv"):
    model.eval()
    all_labels = []
    all_preds = []
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Load the dataset and log its name
        dataset_path = r'Rice_Leaf_Diease\TrainV3'  # Define your dataset path here
        ...
        # Initialize ResNet50 model for supervised learning
        logging.info("Initialize ResNet50 model for supervised learning");
        ...
        logging.info("Starting supervised training with ResNet-50")
        train_supervised(model, train_loader, optimizer, criterion, num_epochs=10, device=device)
